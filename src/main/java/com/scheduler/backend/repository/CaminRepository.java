...
import com.scheduler.backend.model.Camin;
...
public class CaminRepository implements Repository<Camin> {
	...
	public List<Camin> findAll() {// return a list <Camin> with all of the Camine
		Session databaseSession = HibernateService.getSessionFactory().openSession();
		Query query = databaseSession.createQuery("from com.scheduler.backend.model.Camin C");
	...
	{
		Session databaseSession = HibernateService.getSessionFactory().openSession();
		Query query = databaseSession.createQuery("from com.scheduler.backend.model.Camin C WHERE C.admin = :param");
	...
	{
		Session databaseSession = HibernateService.getSessionFactory().openSession();
		Query query = databaseSession.createQuery("from com.scheduler.backend.model.Camin C WHERE C.adress = :param");
	...
	public Camin findByNumber(Integer number) {
		Session databaseSession = HibernateService.getSessionFactory().openSession();
		databaseSession.beginTransaction();
		Query query = databaseSession.createQuery("from com.scheduler.backend.model.Camin C WHERE C.number= :nr"); 
